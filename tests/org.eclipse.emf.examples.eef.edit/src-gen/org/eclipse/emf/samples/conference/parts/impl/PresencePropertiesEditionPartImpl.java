 ...
 * 
 */
public class PresencePropertiesEditionPartImpl extends CompositePropertiesEditingPart implements SWTPropertiesEditingPart, PresencePropertiesEditionPart {
	 ...
	 * 
	 */
	public PresencePropertiesEditionPartImpl(PropertiesEditingComponent editionComponent) {
	...
	protected Composite createAssistsAdvancedReferencesTable(Composite parent) {
		...
		this.assists.setHelpText(propertiesEditingComponent.getHelpContent(ConferenceViewsRepository.Presence.Talks.assists, ConferenceViewsRepository.SWT_KIND));
		this.assists.createControls(parent);
		this.assists.addSelectionListener(new SelectionAdapter() {
			
			public void widgetSelected(SelectionEvent e) {
				if (e.item != null && e.item.getData() instanceof EObject) {
					propertiesEditingComponent.firePropertiesChanged(new PropertiesEditingEventImpl(PresencePropertiesEditionPartImpl.this, ConferenceViewsRepository.Presence.Talks.assists, PropertiesEditingEventImpl.CHANGE, PropertiesEditingEventImpl.SELECTION_CHANGED, null, e.item.getData()));
				}
			}
		...
		"assists", propertiesEditingComponent.getEditingContext().getAdapterFactory(), current.eResource()) {
			...
			public void process(IStructuredSelection selection) {
				for (Iterator<?> iter = selection.iterator(); iter.hasNext();) {
					...
					propertiesEditingComponent.firePropertiesChanged(new PropertiesEditingEventImpl(PresencePropertiesEditionPartImpl.this, ConferenceViewsRepository.Presence.Talks.assists,
	...
	protected void moveAssists(EObject element, int oldIndex, int newIndex) {
		propertiesEditingComponent.firePropertiesChanged(new PropertiesEditingEventImpl(PresencePropertiesEditionPartImpl.this, ConferenceViewsRepository.Presence.Talks.assists, PropertiesEditingEventImpl.COMMIT, PropertiesEditingEventImpl.MOVE, element, newIndex));
		assists.refresh();
	}
	...
	protected void removeFromAssists(EObject element) {
		propertiesEditingComponent.firePropertiesChanged(new PropertiesEditingEventImpl(PresencePropertiesEditionPartImpl.this, ConferenceViewsRepository.Presence.Talks.assists, PropertiesEditingEventImpl.COMMIT, PropertiesEditingEventImpl.REMOVE, null, element));
		assists.refresh();
	}
