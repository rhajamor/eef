 ...
 * 
 */
public class PresencePropertiesEditionPartForm extends CompositePropertiesEditingPart implements FormPropertiesEditingPart, PresencePropertiesEditionPart {
	 ...
	 * 
	 */
	public PresencePropertiesEditionPartForm(PropertiesEditingComponent editionComponent) {
	...
	public void createControls(final FormToolkit widgetFactory, Composite view) {
		CompositionSequence presenceStep = new BindingCompositionSequence(propertiesEditingComponent);
		presenceStep
	...
	protected Composite createAssistsReferencesTable(FormToolkit widgetFactory, Composite parent) {
		...
		this.assists.setHelpText(propertiesEditingComponent.getHelpContent(ConferenceViewsRepository.Presence.Talks.assists, ConferenceViewsRepository.FORM_KIND));
		this.assists.createControls(parent, widgetFactory);
		this.assists.addSelectionListener(new SelectionAdapter() {
			
			public void widgetSelected(SelectionEvent e) {
				if (e.item != null && e.item.getData() instanceof EObject) {
					propertiesEditingComponent.firePropertiesChanged(new PropertiesEditingEventImpl(PresencePropertiesEditionPartForm.this, ConferenceViewsRepository.Presence.Talks.assists, PropertiesEditingEventImpl.CHANGE, PropertiesEditingEventImpl.SELECTION_CHANGED, null, e.item.getData()));
				}
			}
	...
	protected void addAssists() {
		...
		"assists", propertiesEditingComponent.getEditingContext().getAdapterFactory(), current.eResource()) {
			@Override
			public void process(IStructuredSelection selection) {
				for (Iterator<?> iter = selection.iterator(); iter.hasNext();) {
					EObject elem = (EObject) iter.next();
					propertiesEditingComponent.firePropertiesChanged(new PropertiesEditingEventImpl(PresencePropertiesEditionPartForm.this, ConferenceViewsRepository.Presence.Talks.assists,
	...
	protected void moveAssists(EObject element, int oldIndex, int newIndex) {
		propertiesEditingComponent.firePropertiesChanged(new PropertiesEditingEventImpl(PresencePropertiesEditionPartForm.this, ConferenceViewsRepository.Presence.Talks.assists, PropertiesEditingEventImpl.COMMIT, PropertiesEditingEventImpl.MOVE, element, newIndex));
		assists.refresh();
	}
	...
	protected void removeFromAssists(EObject element) {
		propertiesEditingComponent.firePropertiesChanged(new PropertiesEditingEventImpl(PresencePropertiesEditionPartForm.this, ConferenceViewsRepository.Presence.Talks.assists, PropertiesEditingEventImpl.COMMIT, PropertiesEditingEventImpl.REMOVE, null, element));
		assists.refresh();
	}
