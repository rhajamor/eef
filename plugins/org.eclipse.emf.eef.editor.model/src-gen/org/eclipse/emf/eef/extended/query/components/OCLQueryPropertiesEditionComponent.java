	...
	public void updateSemanticModel(final PropertiesEditingEvent event) {
		...
		if (QueryViewsRepository.OCLQuery.Properties.context == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditingEventImpl.SET)  {
				...
				EReferencePropertiesEditingContext context = new EReferencePropertiesEditingContext(editingContext, this, contextSettings, editingContext.getAdapterFactory());
