...
import org.eclipse.emf.eef.runtime.ui.filters.PropertiesEditingPartFilter;
 ...
 */
public class PropertiesEditingViewer extends StructuredViewer {
	 ...
	 */
	public PropertiesEditingViewer(Composite container, ResourceSet allResources, int style, int kind) {
	 ...
	 *            the kind of the part
	 */
	public PropertiesEditingViewer(Composite container, ResourceSet allResources, int kind) {
	...
	public void addPropertiesListener(PropertiesEditingListener listener) {
		if (getContentProvider() != null)
			((PropertiesEditingContentProvider)getContentProvider()).addPropertiesListener(listener);
	}
	 ...
	 */
	private boolean selectPart(String key, PropertiesEditingPart propertiesEditingPart) {
		boolean result = true;
		if (filters != null) {
			boolean select = true;
			for (int j = 0; j < filters.size(); j++) {
				ViewerFilter viewerFilter = filters.get(j);
				if (viewerFilter instanceof PropertiesEditingPartFilter) {
					select = viewerFilter.select(this, key, propertiesEditingPart);
					if (!select) {
						result = false;
						break;
					}
	...
	public Diagnostic validateValue(PropertiesEditingEvent event) {
		if (getContentProvider() != null)
			return ((PropertiesEditingContentProvider)getContentProvider()).validateValue(event);
		return null;
	}
	 ...
	 * @return
	 */
	public CompositePropertiesEditingPart getSelectedPart() {
		PropertiesEditingContentProvider propertiesEditingContentProvider = (PropertiesEditingContentProvider)getContentProvider();
		String selectedPartTitle = folder.getSelection().getText();
		return (CompositePropertiesEditingPart)propertiesEditingContentProvider.getPropertiesEditingPart(kind, selectedPartTitle);
	}
	...
	protected void initControl() {
		if (getContentProvider() != null) {
			PropertiesEditingContentProvider propertiesEditingProvider = (PropertiesEditingContentProvider)getContentProvider();
			// first set initState to true to not handle changes yet
			initState = true;
			String[] partsList = propertiesEditingProvider.partsList();
			initTabbedControl(propertiesEditingProvider, partsList);
			initState = false;
		}
	}

	/**
	 * Initialize the control of the viewer in "Tabbed case". Defines one tab for each PropertiesEditingPart
	 * of the component.
	 * 
	 * @param propertiesEditingProvider
	 * @param partsList
	 */
	protected void initTabbedControl(PropertiesEditingContentProvider propertiesEditingProvider, String[] partsList) {
		resetTab();
		...
		for (int i = 0; i < partsList.length; i++) {
			...
			PropertiesEditingPart part = propertiesEditingProvider.getPropertiesEditingPart(kind,
					nextComponentKey);
			if (selectPart(nextComponentKey, part)) {
				selectedParts.add(nextComponentKey);
				addPartTab(propertiesEditingProvider, part, nextComponentKey);
			} else {
	...
	/**
	 * Create a new tab in the folder for a given PropertiesEditingPart.
	 * 
	 * @param propertiesEditingProvider the EEF properties provider
	 * @param key
	 *            the key of the part
	 * @param tabText
	 *            the title of the tab
	 */
	private void addPartTab(PropertiesEditingContentProvider propertiesEditingProvider, PropertiesEditingPart part, String key) {
		Composite editComposite = null;
		if (part instanceof SWTPropertiesEditingPart)
			editComposite = ((SWTPropertiesEditingPart)part).createFigure(folder);
		if (part instanceof FormPropertiesEditingPart) {
			Assert.isNotNull(toolkit, EEFRuntimeUIMessages.PropertiesEditingViewer_widget_factory_not_defined);
			editComposite = ((FormPropertiesEditingPart)part).createFigure(folder, toolkit);
		}
		if (editComposite != null) {
			if (allResources == null && getEObjectFromInput() != null)
				propertiesEditingProvider.initPart(propertiesEditingProvider.translatePart(key), kind,
	...
	public void refreshTab() {
		...
		PropertiesEditingContentProvider propertiesEditingContentProvider = (PropertiesEditingContentProvider)getContentProvider();
		propertiesEditingContentProvider.initPart(propertiesEditingContentProvider.translatePart(folder.getSelection().getText()), kind, ((EObject)getEObjectFromInput()));
	}
