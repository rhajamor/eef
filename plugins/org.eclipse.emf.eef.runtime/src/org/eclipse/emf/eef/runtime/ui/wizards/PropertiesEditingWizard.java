 ...
 */
public class PropertiesEditingWizard extends Wizard {
	 ...
	 *            the eObject to edit
	 */
	public PropertiesEditingWizard(PropertiesEditingContext editionContext, AdapterFactory adapterFactory,
			EObject eObject) {
	 ...
	 */
	public PropertiesEditingWizard(PropertiesEditingContext editingContext, AdapterFactory adapterFactory,
	...
	public EditingDomain getEditingDomain() {
		return editingContext instanceof DomainPropertiesEditingContext ? ((DomainPropertiesEditingContext)editingContext)
...
//		PropertiesContextService.getInstance().pop();
	...
	protected class ElementCreationWizardPage extends WizardPage {
		...
		protected ElementCreationWizardPage() {
			super(EEFRuntimeUIMessages.PropertiesEditingWizard_creation_page_key);
			this.setTitle(EEFRuntimeUIMessages.PropertiesEditingWizard_creation_page_title);
			this.setDescription(EEFRuntimeUIMessages.PropertiesEditingWizard_creation_page_description);
		}
		...
		public void createControl(Composite parent) {
			...
			if (editingContext instanceof DomainPropertiesEditingContext) {
				instanciableTypesInHierarchy = EEFUtils.allTypeFor(eReference, ((DomainPropertiesEditingContext)editingContext).getEditingDomain());
				editingContext = null;
			} else {
			...
			for (final EClass eClass : instanciableTypesInHierarchy) {
				...
				button.addSelectionListener(new SelectionAdapter() {
					...
					public void widgetSelected(SelectionEvent e) {
						if (editingContext instanceof EReferencePropertiesEditingContext && ((EReferencePropertiesEditingContext)editingContext).getSettings() != null) {
							EEFEditorSettings settings = ((EReferencePropertiesEditingContext)editingContext).getSettings();
							if (settings instanceof ReferencesTableSettings) {
								((ReferencesTableSettings) ((EReferencePropertiesEditingContext)editingContext).getSettings()).removeFromReference(eObject);
							}
			...
			if (buttons.size() > 0) {
				...
				if (editingContext instanceof EReferencePropertiesEditingContext && ((EReferencePropertiesEditingContext)editingContext).getSettings() != null) {
					EEFUtils.putToReference(((EReferencePropertiesEditingContext)editingContext).getSettings(), eObject);
				}			
			} else {
	...
	protected class EditPropertyWizardPage extends WizardPage implements PropertiesEditingListener {

		protected PropertiesEditingViewer viewer;

		protected EditPropertyWizardPage() {
			super(EEFRuntimeUIMessages.PropertiesEditingWizard_main_page_key);
		}
		...
		public void createControl(Composite parent) {
			try {
				...
				viewer = new PropertiesEditingViewer(container, resourceSet, 0);
				viewer.setDynamicTabHeader(true);
				viewer.setContentProvider(new PropertiesEditingContentProvider(adapterFactory, PropertiesEditingComponent.BATCH_MODE));
				scrolledContainer.setContent(container);
				setControl(viewer.getControl());
			} catch (InstantiationException e) {
				EEFRuntimePlugin.getDefault().logError(
						EEFRuntimeUIMessages.PropertiesEditingWizard_error_wizard_live_mode, e);
			}
		}

		public void setInput(EObject eObject) {
			this.setTitle(eObject.eClass().getName());
			this.setDescription(EEFRuntimeUIMessages.PropertiesEditingWizard_main_page_description + eObject.eClass().getName());
