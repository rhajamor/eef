	...
	public void updateSemanticModel(final PropertiesEditingEvent event) {
		...
		if (MappingViewsRepository.ElementBindingReference.Reference.binding == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditingEventImpl.SET)  {
				...
				EReferencePropertiesEditingContext context = new EReferencePropertiesEditingContext(editingContext, this, bindingSettings, editingContext.getAdapterFactory());
